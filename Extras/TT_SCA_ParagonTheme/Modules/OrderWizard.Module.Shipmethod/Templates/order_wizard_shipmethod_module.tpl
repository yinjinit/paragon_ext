...
<script>
	//Deon Doughty - no Module in Extensions Just did a quick find and replace.
	jQuery(document).ready(function() {
		jQuery('[data-action="select-delivery-option"] option').each(function() {
			var text = jQuery(this).html();
			var text_flag = text.search("Free! -");
			if(text_flag != -1){
				jQuery(this).html(text.replace('Free! -', '').replace('#', ''));
			}
		});
	});
</script>
