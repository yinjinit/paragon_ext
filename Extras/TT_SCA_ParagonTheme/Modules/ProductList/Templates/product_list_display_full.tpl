<tr class="{{#if isChecked}}active{{/if}}" data-id="{{lineId}}" data-item-id="{{itemId}}"
	...
	<td class="product-list-display-full-details">
		...
		<div class="product-list-display-full-price">
			<span>MPN: {{mpn}} :&nbsp;<div data-view="ItemViews.Price" style="flex: -1"></div></span>
		</div>
	...
	<td class="product-list-display-full-actions">
		{{#if showEdit}}
		<button class="product-list-display-full-edit" data-action="edit-item"
			data-toggle="show-in-modal">{{translate 'Edit'}}</button>
		{{/if}}
		{{#if showMoveAction}}
