...
<tr id="{{lineId}}" data-item-id="{{itemId}}" data-type="order-item" {{#if showGeneralClass}} class="{{generalClass}}" {{/if}} >
	<td class="col-md-3 thumbnail-padding">
		<div class="transaction-line-cell-item-thumbnail" name="transaction-line-cell-item-thumbnail">
			
			{{#if isNavigable}}
			<a {{{linkAttributes}}} class="transaction-line-views-cell-actionable-name-link">
				{{#if item.custitem_item_thumbnail_url}}
					<img src="{{resizeImage item.custitem_item_thumbnail_url 'thumbnail'}}?resizeh=60" alt="{{thumbnail.altimagetext}}" />
				{{else}}
	...
	<td class="transaction-line-views-cell-actionable-expanded-table-middle col-md-9">
		...
		{{else}}
				<span class="transaction-line-views-cell-actionable-expanded-name-viewonly">{{item.displayname}}</span>
		{{/if}}
		</div>
