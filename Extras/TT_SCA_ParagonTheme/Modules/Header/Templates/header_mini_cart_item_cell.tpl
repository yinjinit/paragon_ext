<li class="header-mini-cart-item-cell" data-item-id="{{itemId}}" data-item-type="{{itemType}}">
	...
	<a {{{linkAttributes}}}>
    	...
    	{{/if}}
    	{{#if line.item.custitem_item_thumbnail_url}}
			<img src="{{resizeImage line.item.custitem_item_thumbnail_url 'tinythumb'}}?resizeh=60" alt="{{thumbnail.altimagetext}}">
		{{else}}
	...
	<div class="header-mini-cart-item-cell-details">
		<ul>
			...
			<li class="header-mini-cart-item-cell-product-title">
				<a {{{linkAttributes}}} class="header-mini-cart-item-cell-title-navigable">{{line.item.displayname}}</a>
			</li>
			...
			{{else}}
				<li>{{translate '<a data-touchpoint="login" data-hashtag="login-register" origin-hash="" href="#">Login</a> to see price'}}</li>
			{{/if}}
