    ...
    <div class="quote-details-view">
                ...
                <div class="quote-details-header-col-left">
                    ...
                    <div class="address-edit-fields-group" data-input="name" data-validation="control-group">
                        <label class="address-edit-fields-group-label" for="name">
                            {{translate 'Name:'}} <span class="address-edit-fields-group-label-required">*</span>
                        </label>
                        <div  class="address-edit-fields-group-form-controls" data-validation="control">
                            <input type="text" class="address-edit-fields-group-input" id="name" name="name" value="{{model.customname}}">
                        </div>
                    </div>
                    {{else}}
                        <p class="quote-details-header-info-request-date">
                            <span class="quote-details-header-label-request-date">{{translate 'Name: '}}</span>
                            <span class="quote-details-header-value-date">{{model.customname}}</span>
                        </p>
                    {{/if}}
                    {{#if isOpen}}
                    <div class="address-edit-fields-group" data-input="notes" data-validation="control-group">
                        ...
                        <div  class="address-edit-fields-group-form-controls" data-validation="control">
                            <textarea class="address-edit-fields-group-input" id="notes" name="notes">{{model.customnotes}}</textarea>
                        </div>
                    </div>
                    {{else}}
                        <p class="quote-details-header-info-request-date">
                            <span class="quote-details-header-label-request-date">{{translate 'Notes: '}}</span>
                            <span class="quote-details-header-value-date">{{model.customnotes}}</span>
                        </p>
                    {{/if}}
                    {{#unless isOpen}}
                        <p class="quote-details-header-info-request-date">
                    ...
                    {{#if removedItem}}
                        <p class="quote-edit-removed-item">
                            ...
                            <a href="#" data-action="undo-remove"><b>{{translate 'Click Here to Undo'}}</b></a>
                        </p>
                    {{/if}}
                    <p class="quote-edit-remove-limit-item">
                        {{translate 'Item cannot be removed, Quotes must to have at least on item' }}
                    </p>
                </div>
        ...
        <div class="quote-details-row">
            <div class="quote-details-content-col">
                ...
                <div class="quote-details-accordion-divider">
                    <div class="quote-details-accordion-head">
                        <a class="quote-details-accordion-head-toggle" data-toggle="collapse" data-target="#quote-shipping-info" aria-expanded="false" aria-controls="#quote-billing-info">
                            {{translate 'Shipping Information'}}
                            <i class="quote-details-accordion-toggle-icon"></i>
                        </a>
                    </div>
                    <div class="quote-details-accordion-body collapse in" id="quote-shipping-info" role="tabpanel" data-target="quote-shipping-info">
                        <div class="quote-details-accordion-container">
            ...
            <div class="quote-details-summary-col">
                ...
                <div class="quote-details-row-fluid">
                    ...
                    {{#if isOpen}}
                        ...
                        {{/unless}}
                        {{#if hasPermission}}
                            <button data-action="save-quote" class="quote-details-button-save">{{translate 'Save Quote'}}</button>
                            <button data-action="convert-to-order" data-url="{{reviewQuoteURL}}" class="quote-details-button-download-pdf" {{#unless model.purchasablestatus.isPurchasable}}disabled{{/unless}}>{{translate 'Convert Quote to Order'}}</button>
                        {{/if}}
                    {{/if}}
                    <button data-action="copy-quote" class="quote-details-button-tertiary">{{translate 'Make a Copy'}}</button>
                    <button data-action="open-share-modal" class="quote-details-button-tertiary">{{translate 'Share Quote'}}</button>
                    <a href="{{pdfUrl}}" target="_blank" class="quote-details-button-tertiary">{{translate 'Download as PDF'}}</a>
                </div>
...
jQuery(document).ready(function(){
    ...
    jQuery('.transaction-line-views-cell-navigable-details a, .transaction-line-views-cell-navigable-product-name a').on('click tap', function(e){
        e.preventDefault();
        var urlParse = jQuery(this).attr('data-url');
        console.log('data-url');
        console.log(urlParse);
    ...
    jQuery('.transaction-line-views-cell-navigable-details a, .transaction-line-views-cell-navigable-product-name a').on('hover', function(e){
        e.preventDefault();
        e.stopPropagation();
    });
