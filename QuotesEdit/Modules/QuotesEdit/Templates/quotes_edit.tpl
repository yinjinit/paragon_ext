<a href="/quotes" class="quote-details-header-back-btn">{{translate
  '&lt; Back to quotes'}}</a>
    ...
    <div class="quote-details-header-information">
      <div class="quote-details-row">
        <div class="quote-details-header-col-left">
          <div data-type="alert-placeholder-copy"></div>
          {{#if quoteSaved}}
          <p class="quote-edit-save-quote">
            {{translate 'Quote saved successfully'}}
          </p>
          {{/if}}
          {{#if isOpen}}
          <div class="address-edit-fields-group" data-input="name"
            data-validation="control-group">
            <label class="address-edit-fields-group-label" for="name">
              {{translate 'Name:'}} <span
              class="address-edit-fields-group-label-required">*</span>
            </label>
            <div class="address-edit-fields-group-form-controls"
              data-validation="control">
              <input type="text" class="address-edit-fields-group-input"
                id="name" name="name" value="{{model.customname}}">
            </div>
          </div>
          {{else}}
          <p class="quote-details-header-info-request-date">
              <span class="quote-details-header-label-request-date">{{translate
                'Name: '}}</span>
            <span
              class="quote-details-header-value-date">{{model.customname}}</span>
          </p>
          {{/if}}
          {{#if isOpen}}
          <div class="address-edit-fields-group" data-input="notes"
            data-validation="control-group">
            <label class="address-edit-fields-group-label" for="notes">
              {{translate 'Notes'}} <span
              class="address-edit-fields-group-label-required">*</span>
            </label>
            <div class="address-edit-fields-group-form-controls"
              data-validation="control">
                <textarea class="address-edit-fields-group-input" id="notes"
                  name="notes">{{model.customnotes}}</textarea>
            </div>
          </div>
          {{else}}
          <p class="quote-details-header-info-request-date">
              <span class="quote-details-header-label-request-date">{{translate
                'Notes: '}}</span>
            <span
              class="quote-details-header-value-date">{{model.customnotes}}</span>
          </p>
          {{/if}}
          {{#unless isOpen}}
          <p class="quote-details-header-info-request-date">
              <span class="quote-details-header-label-request-date">{{translate
                'Request date: '}}</span>
            <span
              class="quote-details-header-value-date">{{model.trandate}}</span>
          </p>
          <p class="quote-details-header-info-expiration-date">
              <span
                class="quote-details-header-info-expiration-date-label">{{translate
                'Expiration date: ' }}</span>
            {{#if hasDuedate}}
            <span
              class="quote-details-header-info-expiration-date-value">{{duedate}}</span>

            {{#if model.isOverdue }}
            <i
              class="quote-details-header-info-expiration-date-icon-overdue"></i>
            {{else}}
                  {{#if model.isCloseOverdue}}
            <i
              class="quote-details-header-info-expiration-date-icon-closeoverdue"></i>
            {{/if}}
                {{/if}}
              {{else}}
            <span
              class="quote-details-header-info-expiration-date-value">{{translate
                  'Not specified'}}</span>
            {{/if}}
          </p>
          {{/unless}}
          {{#if removedItem}}
          <p class="quote-edit-removed-item">
            {{translate 'Item $(0) was removed from the qoute' removedItem}}
            <a href="#" data-action="undo-remove"><b>{{translate
                'Click Here to Undo'}}</b></a>
          </p>
          {{/if}}
          <p class="quote-edit-remove-limit-item">
            {{translate
              'Item cannot be removed, Quotes must to have at least on item' }}
          </p>
        </div>
    ...
    <div class="quote-details-row">
      <div class="quote-details-content-col">
        ...
        <div class="quote-details-accordion-divider">
          ...
          <div class="quote-details-accordion-head">
            <a class="quote-details-accordion-head-toggle collapsed"
              data-toggle="collapse" data-target="#quote-comments"
              aria-expanded="false" aria-controls="#quote-comments">
          ...
          <div class="quote-details-accordion-body collapse"
            id="quote-comments" role="tabpanel" data-target="quote-comments">
        ...
        <div class="quote-details-accordion-divider">
          <div class="quote-details-accordion-head">
            <a class="quote-details-accordion-head-toggle"
              data-toggle="collapse" data-target="#quote-shipping-info"
              aria-expanded="false" aria-controls="#quote-billing-info">
              {{translate 'Shipping Information'}}
              <i class="quote-details-accordion-toggle-icon"></i>
            </a>
          </div>
          <div class="quote-details-accordion-body collapse in"
            id="quote-shipping-info" role="tabpanel"
            data-target="quote-shipping-info">
            <div class="quote-details-accordion-container">
        ...
        <div class="quote-details-accordion-divider">
          ...
          <div class="quote-details-accordion-head">
            <a class="quote-details-accordion-head-toggle collapsed"
              data-toggle="collapse" data-target="#quote-messages"
              aria-expanded="false" aria-controls="#quote-messages">
          ...
          <div class="quote-details-accordion-body collapse"
            id="quote-messages" role="tabpanel" data-target="quote-messages">
            <div class="quote-details-accordion-container">
        ...
        <div class="quote-details-disclaimer-bottom-content">
          {{#if hasSalesrep}}
          <small class="quote-details-disclaimer-message">{{translate
              'For immediate assistance contact <strong>$(0)</strong> at <strong>$(1)</strong>. For additional information, send an email to <strong>$(2)</strong>.'
              salesrepName salesrepPhone salesrepEmail}}</small>
          {{else}}
          <small
            class="quote-details-disclaimer-message">{{{disclaimer}}}</small>
          {{/if}}
        </div>
      </div>
      ...
      <div class="quote-details-summary-col">
        <div class="quote-details-summary-container">
          ...
          <div class="quote-details-summary-grid">
            <div class="quote-details-summary-promocode-text-success">
                <span
                  class="quote-details-summary-promocode-code">#{{model.promocode.code}}</span>
            </div>
          </div>
          {{/if}}
        ...
        <div class="quote-details-row-fluid">
          ...
          <div data-type="quote-details-and-order-msg-placeholder">
            <div class="quote-details-msg">
              ...
              {{/if}}
                  ...
                  {{#if hasSalesrep}}
              <p>{{translate
                      'To place the order please contact <strong>$(0)</strong> at <strong>$(1)</strong> or send an email to <strong>$(2)</strong>'
                      salesrepName salesrepPhone salesrepEmail}}</p>
              {{else}}
          ...
          {{/unless}}
            {{#if hasPermission}}
          <button data-action="save-quote"
            class="quote-details-button-save">{{translate
                'Save Quote'}}</button>
          <button data-action="convert-to-order"
            data-url="{{reviewQuoteURL}}"
            class="quote-details-button-download-pdf" {{#unless
                model.purchasablestatus.isPurchasable}}disabled{{/unless}}>{{translate
                'Convert Quote to Order'}}</button>
          {{/if}}
          {{/if}}
          <button data-action="copy-quote"
            class="quote-details-button-tertiary">{{translate
            'Make a Copy'}}</button>
          <button data-action="open-share-modal"
            class="quote-details-button-tertiary">{{translate
            'Share Quote'}}</button>
...
<script>
  jQuery(document).ready(function() {
    ...
    jQuery('.cart-lines-thumbnail a, .cart-lines-name a')
    ...
    jQuery(
      '.transaction-line-views-cell-navigable-details a, .transaction-line-views-cell-navigable-product-name a')
      .on('click tap', function(e) {
        e.preventDefault();
        var urlParse = jQuery(this).attr('data-url');
        console.log('data-url');
