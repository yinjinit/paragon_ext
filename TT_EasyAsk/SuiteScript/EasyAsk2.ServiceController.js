...
,  function(
  ...
  )
  {
    ...
    return ServiceController.extend({
      ...
      name: 'EasyAsk2.ServiceController'
    ...
    ,  get: function get(price,page_results_quantity,pageNumber,search_keyword)
      {  
        var price= this.request.getParameter('price');
        var page_results_quantity= this.request.getParameter('page_results_quantity');
        var pageNumber= this.request.getParameter('pageNumber');
        var search_keyword=this.request.getParameter('keyword');
        nlapiLogExecution("audit","search_keyword is in EasyAsk2",JSON.stringify(price));
        nlapiLogExecution("audit","price is in EasyAsk2",JSON.stringify(price));
            nlapiLogExecution("audit","page_results_quantity is in EasyAsk2",JSON.stringify(page_results_quantity));
            nlapiLogExecution("audit","pageNumber is in EasyAsk2",JSON.stringify(pageNumber));
        var ctx = ModelsInit.context;
        nlapiLogExecution("audit","CategoriesServiceController.fullurl outside",JSON.stringify(CategoriesServiceController.fullurl));
  ...
  fullurlCategory=fullurlCategory.replace("/products/","").toString() || fullurlCategory.replace("/sca-dev-2019-2/shopping-local.ssp#products/","").toString();
  nlapiLogExecution("audit","fullurlCategory after replacement in EXTENSION",JSON.stringify(fullurlCategory));
  const easyAskFormattedType = [
      ...
      nlapiLogExecution("audit","fullurlCategory before assignement in EXTENSION",JSON.stringify(fullurlCategory));
      ...
      nlapiLogExecution("audit","fullurlCategoryVectorVector after assignement",JSON.stringify(fullurlCategoryVectorVector));
        ...
        var url = "https://ea.paragonmicro.com:9203/EasyAsk/apps/Advisor.jsp?dct=paragonmicro&indexed=1&oneshot=1&disp=json&RequestAction=navbar&RequestData=page"+pageNumber+"&CatPath="+fullurlCategoryVectorVector+"&ResultsPerPage="+page_results;
          ...
          return CategoriesServiceController.getEasyAsk(price,page_results_quantity,pageNumber,search_keyword)
