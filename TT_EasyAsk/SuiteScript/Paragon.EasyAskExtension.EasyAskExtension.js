// Paragon.EasyAskExtension.EasyAskExtension.js
// Load all your starter dependencies in backend for your extension here
// ----------------

define('Paragon.EasyAskExtension.EasyAskExtension'
,	[
		'SC.Model',
...
,	function (
    SCModel,
		EasyAskExtensionServiceController,
		_,
		ProfileModel,
		LiveOrderModel,
...
{
	'use strict';
  nlapiLogExecution("audit","- LiveOrderOut",JSON.stringify(LiveOrderModel));


  return SCModel.extend({

    name: 'Paragon.EasyAskExtension.EasyAskExtension',

/*
   set: function set (fullurl)
        {
  //nlapiLogExecution("audit","ServiceController",JSON.stringify(CategoriesServiceController.fullurl));
  //nlapiLogExecution("audit","ServiceController get",JSON.stringify(CategoriesServiceController.get()));
  if(fullurl && fullurl !== null) {
  try{
   nlapiLogExecution("audit","- LiveOrderIn",JSON.stringify(LiveOrderModel.getSummary()));
  //var fullurlCategory = CategoriesServiceController.fullurl;
  nlapiLogExecution("audit","fullurlCategory outside",JSON.stringify(fullurl));
  fullurlCategory=fullurl.replace("/products/","").toString();
  ...
  const easyAskFormattedType = [
      ...
      nlapiLogExecution("audit","fullurlCategory before assignement",JSON.stringify(fullurlCategory));
                ...
                var fullurlCategoryVector = (_.find(easyAskFormattedType, function isValue(system) { 
                    return system.item == fullurlCategory;})).type;
      ...
      nlapiLogExecution("audit","fullurlCategoryVectorVector after assignement",JSON.stringify(fullurlCategoryVectorVector));
                ...
                var url = "https://ea.paragonmicro.com:9114/EasyAsk/apps/Advisor.jsp?dct=paragonmicro&indexed=1&oneshot=1&disp=json&CatPath="+fullurlCategory+"&ResultsPerPage=24";
                ...
                nlapiLogExecution("audit","LiveOrderSetTransac",JSON.stringify(LiveOrderModel));
                nlapiLogExecution("audit","easyask_info",JSON.stringify(easyask_info));
                //var easyeasy = {custbody_easyaskinfo: easyask_info};
                nlapiLogExecution("audit","ModelsInit",JSON.stringify(easyeasy));
                


                //ModelsInit.order.setCustomFieldValues(easyask_info);
                

                //LiveOrderModel.setTransactionBodyField(options);
                
                //this.get(easyask_info);
                
                //var easyeasy = {custbody_easyaskinfo: easyask_info};
                //ModelsInit.order.setCustomFieldValues(options);
    ...
    ,get: function get(easyask_info){
          const easyask_info2 = easyask_info;
          nlapiLogExecution("audit","setting easyask_info",JSON.stringify(easyask_info2));
          //LiveOrderModel.setTransactionBodyField(easyask_info2);
