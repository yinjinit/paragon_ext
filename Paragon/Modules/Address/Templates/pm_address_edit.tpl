<section class="address-edit">
	{{#unless isInModalOrHideHeader}}
		...
		{{#if isCollectionEmpty}}
			<p>{{translate
				'For faster checkouts, please enter an address below.'}}</p>
		{{/if}}
	{{/unless}}
	...
	<form class="address-edit-form" action="addressbook.ss" method="POST">
		...
		{{#if showFooter}}
			<div
				class="{{#if isInModal}}address-edit-footer{{else}}form-actions{{/if}}">
				...
				{{#unless hideSelectButton }}
					<button class="address-edit-form-button-submit"
						style="margin-left: 10px;" data-action="update_select">
						{{translate 'Update & Select'}}
					</button>
				{{/unless}}

				{{#if isInModalOrCollectionNotEmpty}}
					<button class="address-edit-form-button-cancel" data-dismiss="modal"
						data-action="reset">
						{{translate 'Cancel'}}
					</button>
				{{/if}}
