<div class="order-wizard-address-module">
	<div class="order-wizard-address-module-show-addresses-container">
		{{#if showTitle}}
			<h3 class="order-wizard-address-module-title">{{title}}</h3>
		{{/if}}
		{{#if isSameAsEnabled}}
			<label class="order-wizard-address-module-checkbox"> <input
					{{#if isSameAsCheckBoxDisable}}disabled="disabled"{{/if}}
				type="checkbox" name="same-as-address" data-action="same-as"
				id="sameAsShipping" value="1"
					{{#if isSameAsSelected}}checked{{/if}}>{{sameAsMessage}}</label>
		{{/if}}

		{{#if showSingleAddressDetails}}
			<div data-view="Single.Address.Details"
				class="order-wizard-address-module-single"></div>
		{{else}}
			{{#if showAddressList}}
				<div id="order-wizard-address-module-placeholder"
						{{#if showManageValue}}data-manage="{{manageValue}}"{{/if}}
					class="order-wizard-address-module-list-placeholder">
					<div class="order-wizard-address-module-address-container">
						<div data-view="Address.Search"></div>
					...
					{{#if showSaveButton}}
						<div class="order-wizard-address-module-form-actions">
							<button type="submit"
								...
								data-action="submit">{{translate 'Save Address'}}</button>
						</div>
					{{/if}}
				</div>
			{{/if}}
