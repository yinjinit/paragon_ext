<div class="order-history-summary-summary-col">
	<div class="order-history-summary-summary-container">
		...
		{{#if showSummaryPickupCost}}
			<p class="order-history-summary-summary-grid-float">
				{{translate 'Pick Up'}}
				<span class="order-history-summary-pickup-label-free"> {{translate
					'FREE'}}</span>
			</p>
		{{/if}}
	...
	<div class="order-history-summary-row-fluid">
		{{#if showReorderAllItemsButton}}
			<!-- REORDER ALL ITEMS -->
			<a href="{{translate
				'<a href="/reorderItems?order_id=$(0)&order_number=$(1)'
				model.internalid model.tranid}}"
				class="order-history-summary-button-reorder">{{translate
				'Reorder All Items'}}</a>
		{{/if}}

		<!-- DOWNLOAD AS PDF -->
		<a href="{{pdfUrl}}" target="_blank"
			class="order-history-summary-button-download-pdf">
			{{translate 'Download PDF'}}
		</a>

		{{#if showRequestReturnButton}}
			<a data-permissions="transactions.tranRtnAuth.2"
				href="/returns/new/{{model.recordtype}}/{{model.internalid}}"
		...
		{{#if showViewInvoiceButton}}
			<a data-permissions="" href="/invoices/{{invoiceModel.internalid}}"
				data-id="{{invoiceModel.internalid}}"
	...
	<div class="order-history-summary-extra-fields">
		{{#if purchaseNumber }}
			<div class="order-history-summary-extra-field">
				<label class="order-history-summary-extra-field-name">{{translate
					'Purchase Order Number'}}:</label>
				<div
					class="order-history-summary-extra-field-content">{{purchaseNumber}}</div>
			</div>
		{{/if}}
		{{#if orderName }}
			<div class="order-history-summary-extra-field">
				<label class="order-history-summary-extra-field-name">{{translate
					'Order Name'}}:</label>
				<div
					class="order-history-summary-extra-field-content">{{orderName}}</div>
			</div>
		{{/if}}
		{{#if costCenter }}
			<div class="order-history-summary-extra-field">
				<label class="order-history-summary-extra-field-name">{{translate
					'Cost Center'}}:</label>
				<div
					class="order-history-summary-extra-field-content">{{costCenter}}</div>
			</div>
		{{/if}}
		{{#if webNotes }}
			<div class="order-history-summary-extra-field">
				<label class="order-history-summary-extra-field-name">{{translate
					'Notes'}}:</label>
				<div
					class="order-history-summary-extra-field-content">{{webNotes}}</div>
			</div>
		{{/if}}
	</div>
</div>
