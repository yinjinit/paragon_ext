...
define('PM.Renewal.Add.View'
  ...
  , function(
    ...
    // @class PM.Renewal.View @extends Backbone.View
    return Backbone.View.extend({

      template: renewal_new_tpl

      , className: 'RenewalModel'
      ...
      , getPrimaryContactId: function() {
        ...
        //611 on production
        jQuery.getJSON('/app/site/hosting/scriptlet.nl?script=611&deploy=1',
      ...
      , changeStart: function(e) {
        ...
        jQuery('#custrecord_ng_rt_start_date_hidden')
          .val(this.formatDate(startValue));

        //this.changeRenewal(e);

        var renewalText = jQuery(
          '[name="custrecord_ng_rt_renewal_period"] :selected').html();

        if (renewalText != 'Custom') {
          var renewalArr = renewalText.split(' ');
          var months = renewalArr[0];
          //console.log(months);
          var start_date = jQuery('[name="custrecord_ng_rt_start_date_hidden"]')
            .val();
          ...
          //console.log(date);
          var resultRenewal = date.getFullYear() + '-' + (
            date.getMonth() + 1
          ) + '-' + date.getDate();
          //console.log(resultRenewal);
          jQuery('#custrecord_ng_rt_renewal_date')
            .val(this.formatDisplayDate(resultRenewal));
          ...
          var reminderText = jQuery(
            '[name="custrecord_ng_rt_reminder_period"] :selected').html();

          if (reminderText != 'Custom') {
            var reminderArr = reminderText.split(' ');
            var days = reminderArr[0];
            //console.log(days);
            var renewal_date = jQuery(
              '[name="custrecord_ng_rt_renewal_date_hidden"]').val();
            ...
            jQuery('#custrecord_ng_rt_reminder_date')
              .val(this.formatDisplayDate(result));
      ...
      , changeReminderDate: function(e) {
        ...
        var remiderPeriodCheck = jQuery(
          '[name="custrecord_ng_rt_reminder_period"]').val();
        //console.log('remiderPeriodCheck: ' + remiderPeriodCheck);

        if (jQuery('[name="custrecord_ng_rt_reminder_date"]').is(':visible') ==
          true) {

          //console.log("Reminder Date is visible");
          var customReminderDate = jQuery('#custrecord_ng_rt_reminder_date')
            .val();

          jQuery('#custrecord_ng_rt_reminder_date_hidden')
            .val(this.formatDate(customReminderDate));
      ...
      , changeReminder: function(e) {
        ...
        var selectedIndex = !changeStart ? this.$(e.target).val() :
          jQuery('#custrecord_ng_rt_reminder_period').val();
        ...
        var checkHidden = jQuery(e.currentTarget)
          .parent()
      ...
      , changeRenewalDate: function(e) {
        //var startValue = jQuery('#custrecord_ng_rt_start_date_hidden').val().split('-');
        var renewalPeriodCheck = jQuery(
          '[name="custrecord_ng_rt_renewal_period"]').val();
        //console.log('renewalPeriodCheck: ' + renewalPeriodCheck);

        if (jQuery('[name ="custrecord_ng_rt_renewal_date"]').is(':visible') ==
          true) {

          //console.log("Renewal Date is visible");
          var customRenewalDate = jQuery('#custrecord_ng_rt_renewal_date')
            .val();

          jQuery('#custrecord_ng_rt_renewal_date_hidden')
            .val(this.formatDate(customRenewalDate));
      ...
      , changeRenewal: function(e) {
        var changeStart = this.$(e.target).data()['action'] == 'change-start';
        var selectedIndex = !changeStart ? this.$(e.target).val() :
          jQuery('#custrecord_ng_rt_renewal_period').val();
        ...
        var checkHiddenStatus = jQuery(e.currentTarget)
          .parent()
          .find('[data-action="items-expander"] a i')
          .toggleClass('icon-minus')
          .end()
      ...
      , customExpander: function(e) {
        ...
        var selectName = jQuery(e.currentTarget)
          .parent()
          .find('[data-action="items-expander"] a i')
          .toggleClass('icon-minus')
          .end()
          .find('.input-case-title')
          .stop()
          .attr('id');
      ...
      , showSuccess: function() {
        ...
        var renewal_link_name = _('Renewal Tracking #$(0)')
            .translate(this.model.get('internalid'))
          ,
          new_renewal_internalid = this.model.get('internalid')
          ,
          renewal_link = '<a href="/reminders/' + new_renewal_internalid +
            '">' + renewal_link_name + '</a>'
          ,
          new_renewal_message = Utils.translate(
            'Good! your <a href="/showreminder/$(0)">$(1)</a> was submitted.',
            new_renewal_internalid, renewal_link_name);
      ...
      , formatDisplayDate: function formatDisplayDate(dateinput) {
        ...
        return parseInt(month, 10) + '/' + parseInt(days, 10) + '/' +
          parseInt(parts[0]);
      ...
      , formatDateInput: function(dateinput) {
        var parts = dateinput.split('-');
        return parseInt(parts[0]) + '-' + parseInt(parts[1], 10) + '-' +
          parseInt(parts[2], 10);
      }
      ...
      , validateReminder: function validateReminder() {
        ...
        var renewal_date = jQuery(
          '[name="custrecord_ng_rt_renewal_date_hidden"]').val();
        var customReminderDate = jQuery(
          '[name="custrecord_ng_rt_reminder_date_hidden"]').val();
        ...
        if ((
          new Date(renewal_date).getTime() < new Date(start_date).getTime()
        )) {
          ...
          new Date(customReminderDate).getTime() <
      ...
      //@method getContext @return PM.Renewal.View.Context
      , getContext: function getContext() {
        //@class PM.Renewal.View.Context
        ...
        for (var r = 0; r < renewals.length; r++) {
          ...
          renewalSelect +=
            '<option value="' + renewals[r].id + '">' + renewals[r].text +
            '</option>\n';
        }
        ...
        var reminders = [
          {id: '1', text: '30 Days Prior'},
          {id: '2', text: '45 Days Prior'},
          {id: '3', text: '60 Days Prior'},
          {id: '4', text: '90 Days Prior'},
          {id: '5', text: '120 Days Prior'},
          {id: '6', text: '180 Days Prior'},
          {id: '7', text: 'Custom'}
        ];
        ...
        for (var i = 0; i < reminders.length; i++) {
          //console.log(renewals[r]);
          remindersSelect +=
            '<option value="' + reminders[i].id + '">' + reminders[i].text +
            '</option>\n';
        }
        ...
        console.log(
          'initialStartDateHiddenParse: ' + initialStartDateHiddenParse);

        var initalRenewalDateHiddenParse = this.addMonths(3,
          initialStartDateHiddenParse);
        var initalReminderHiddenDateParse = this.addDays(30,
          initalRenewalDateHiddenParse);

        //console.log('initalRenewalDateHiddenParse: ' + initalRenewalDateHiddenParse);

        var initialStartDateDisplayParse = this.formatDisplayDate(
          initialStartDateHiddenParse);
        var initalRenewalDisplaylDateParse = this.formatDisplayDate(
          initalRenewalDateHiddenParse);
        var initalReminderDisplayDateParse = this.formatDisplayDate(
          initalRenewalDateHiddenParse);
